...
export const updateProduct = async (req, res) => {
  ...
  try {
    const { id } = req.params;
    const {
      name, description, unit_price, barcode,
      sale_type, unit_cost, has_tax,
      is_iva, is_ieps, ieps_rate,
      category_id, unit_id, is_perishable
    } = req.body;
    
    if (!name && unit_price === undefined && !barcode && sale_type === undefined &&
        unit_cost === undefined && has_tax === undefined && is_iva === undefined &&
        is_ieps === undefined && ieps_rate === undefined) {
